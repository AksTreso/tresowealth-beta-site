...
# Import web search and ClawRouter
...
try:
    ...
    logger.info("ClawRouter client initialized")
except Exception as e:
    client = None
    logger = logging.getLogger(__name__)
    logger.warning(f"Could not initialize ClawRouter client: {e}")
...
class SimpleBatchRewriter:
    ...
    def rewrite_article(self, article: Dict, output_dir: Path) -> Dict:
        ...
        try:
            ...
            # Initialize researcher with web_search and clawrouter_client
    ...
    def _generate_article_from_research(self, topic: str, research_result: Dict) -> str:
        ...
        try:
            if not client:
                logger.error("ClawRouter client not available")
                return None
...
"""
            ...
            # Import Tier from clawrouter_client
    ...
    def _format_statistic(self, stat: Dict) -> str:
        """Format statistic for display, handling truncation and redundancy"""
        ...
        # If no context or context is too short, return just the value
        if not context or len(context) < 15:
            return value

        # Clean markdown formatting from context
        context = re.sub(r'\*\*\*', '', context)  # Remove ***
        context = re.sub(r'\*\*', '', context)    # Remove **
        context = re.sub(r'\*', '', context)      # Remove *
        context = re.sub(r'\n', ' ', context)     # Replace newlines with spaces
        context = re.sub(r'\s+', ' ', context)    # Collapse multiple spaces

        # Check if context starts with truncation artifacts (word fragments only)
        truncation_artifacts = [
            'ort ', 'ut ', ' gr', '** ', 'FMEs*', ' 1', ' 2', ' 3', ' 4', ' 5', ' 6', ' 7', ' 8', ' 9',
            'Context:', 'Ntres', 'Es.', 'Unde'
        ]
        if any(context.startswith(t) for t in truncation_artifacts):
            return value

        # Check context quality - count artifacts
        quality_issues = 0
        if context.count('.') > 3:  # Too many periods (sentence fragments)
            quality_issues += 1
        if context.count(':') > 2:  # Too many colons (markdown lists)
            quality_issues += 1
        if len(context) < 30:  # Too short after cleaning
            quality_issues += 1

        # If too many quality issues, return just the value
        if quality_issues >= 2:
            return value

        # If value not in context, use both directly
        if value not in context:
            clean_context = context.strip()
            if len(clean_context) > 80:
                clean_context = clean_context[:77] + '...'
            return f'{value}: {clean_context}'

        # Value is in context - extract the meaningful part
        value_pos = context.find(value)

        # Extract text after the value (usually more meaningful)
        after_value = context[value_pos + len(value):].strip()
        after_value = after_value.lstrip(':,- ').strip()

        # Only use after_value if it's long enough and doesn't start with artifacts
        if len(after_value) > 15 and not any(after_value.startswith(t) for t in truncation_artifacts):
            clean_context = after_value
        else:
            # After value too short or garbled, try before value
            before_value = context[:value_pos].strip()
            before_value = before_value.rstrip(':,- ').strip()

            if len(before_value) > 15:
                clean_context = before_value
            else:
                # Both parts too short, return just value
                return value

        # Check if clean_context is a sentence fragment (ends with verbs)
        fragment_enders = [
        ...
        if any(clean_context.lower().endswith(e) for e in fragment_enders):
            # Fragment, return just value
            return value

        # Clean up the chosen context
        if len(clean_context) > 80:
            clean_context = clean_context[:77] + '...'

        # Capitalize first letter
        clean_context = clean_context[0].upper() + clean_context[1:] if clean_context else ''

        # Ensure proper ending
        if clean_context and not clean_context[-1] in '.!?;,':
            clean_context += '.'

        return f'{value}: {clean_context}' if clean_context else value

    def _format_regulatory_ref(self, ref: Dict) -> str:
        """Format regulatory reference for display"""
        ...
        if reference and context:
            ...
            if clean_context and not clean_context[-1] in '.!?;,':
                clean_context += '.'
            return f'{reference}: {clean_context}'
        elif reference:
            return reference
        else:
            return 'N/A'
        ...
        <article>
            ...
            <aside class="research-highlights">
                ...
                <div class="statistics">
                    ...
                    <ul>
                        {chr(10).join([f'<li>{self._format_statistic(s)}</li>' for s in statistics[:5]])}
                    </ul>
                </div>
                ...
                <div class="regulatory">
                    ...
                    <ul>
                        {chr(10).join([f'<li>{self._format_regulatory_ref(r)}</li>' for r in regulatory_refs[:3]])}
                    </ul>
                </div>
            </aside>
        </article>
